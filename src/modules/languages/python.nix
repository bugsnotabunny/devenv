...
let
  ...
  libraries = lib.makeLibraryPath (cfg.libraries
    ++ (lib.optional cfg.manylinux.enable
  ...
  package = pkgs.callPackage ../../python-wrapper.nix {
    ...
    makeWrapperArgs = [ "--prefix" "LD_LIBRARY_PATH" ":" libraries ]
  ...
  initVenvScript = ''
    ...
    requirements="${
      lib.optionalString (cfg.venv.requirements != null) "${requirements}"
    }"
      ...
      ${
        ...
        '' else ''
          echo ${package.interpreter} -m venv ${
            if builtins.isNull cfg.version
            || lib.versionAtLeast cfg.version "3.9" then
              "--upgrade-deps"
            else
              ""
          } "$VENV_PATH"
  ...
  initUvScript = ''
    ...
    function _devenv_uv_sync
    {
      ...
      local UV_SYNC_COMMAND=(${cfg.uv.package}/bin/uv sync ${
        lib.escapeShellArgs cfg.uv.sync.arguments
      })
  ...
  initPoetryScript = ''
    ...
    {
      local POETRY_INSTALL_COMMAND=(${cfg.poetry.package}/bin/poetry install --no-interaction ${
        lib.concatStringsSep " " cfg.poetry.install.arguments
      })
...
in {
  options.languages.python = {
    ...
    venv = {
      ...
      requirements = lib.mkOption {
        type =
          lib.types.nullOr (lib.types.either lib.types.lines lib.types.path);
        default = null;
        description = ''
    ...
    poetry = {
      ...
      install = {
        enable =
          lib.mkEnableOption "poetry install during devenv initialisation";
        arguments = lib.mkOption {
          type = lib.types.listOf lib.types.str;
          default = [ ];
          description =
            "Command line arguments pass to `poetry install` during devenv initialisation.";
          internal = true;
        };
  ...
  config = lib.mkIf cfg.enable {
    languages.python.poetry.install.enable =
      lib.mkIf cfg.poetry.enable (lib.mkDefault true);
    languages.python.poetry.install.arguments =
      lib.optional cfg.poetry.install.onlyInstallRootPackage "--only-root"
      ++ lib.optional (!cfg.poetry.install.installRootPackage
        && !cfg.poetry.install.onlyInstallRootPackage) "--no-root"
    ...
    languages.python.package = lib.mkMerge [
      (lib.mkIf (cfg.version != null)
        (nixpkgs-python.packages.${pkgs.stdenv.system}.${cfg.version} or (throw
          "Unsupported Python version, see https://github.com/cachix/nixpkgs-python#supported-python-versions")))
    ];
    ...
    assertions = [{
      ...
      message =
        "Error: Both poetry.install.enable and uv.sync.enable cannot be true simultaneously.";
    }];

    tasks = {
      "devenv:python:virtualenv" =
        lib.mkIf (cfg.venv.enable && !cfg.uv.sync.enable) {
      ...
      "devenv:python:poetry" = lib.mkIf cfg.poetry.install.enable {
        ...
        exports = [ "PATH" ]
          ++ lib.optional cfg.poetry.activate.enable "VIRTUAL_ENV";
        before = [ "devenv:enterShell" ]
